...
-   面向连接：一对一
-   可靠交付：无论网络链路如何变化都可以保证每个报文到达接收端
-   面向字节流：仅仅把上层协议传递来的数据当成字节传输

## TCP报文

![20191129145400104_2106](README.assets/20191129145400104_2106.png)

-   32位端口号：来自哪里（源端口）+传给哪个上层协议或应用程序（目的端口）
-   32位序号：字节编号，***用来解决网络包乱序的问题***
-   32位确认号：用作另一方报文段的响应，值是收到的报文段的序号值+1，***解决丢包的问题***
-   4位头部长度：TCP有多少个32bit
-   6位标志
    -   `URG`：表示紧急指针是否有效
    -   `ACK`：表示确认号是否有效
    ...
    -   `SYN`：同步报文段，建立连接的标志
    -   `FIN`：结束报文段，通知关闭连接
-   16位窗口大小：流量控制窗口，告知对方自己的接收缓冲区还能容纳多少字节的数据，***解决流量控制***
-   16位校验和：CRC算法，（头部+数据）→ 校验和
-   16位紧急指针：发送紧急数据的方法，偏移量+序号=紧急数据下一字节的序号
